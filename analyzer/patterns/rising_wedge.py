"""
Rising Wedge pattern detector (buy signal, 77% win rate).

Project definition: ascending highs and ascending lows that are converging
upward, followed by a breakout above the upper trendline (bullish continuation).

Pattern criteria:
  - At least 3 ascending peaks (higher highs)
  - At least 3 ascending troughs (higher lows)
  - Lower trendline slope > upper trendline slope (lines converge upward)
  - Duration: 20-60 trading days
  - Breakout: price closes above projected upper trendline
"""
...
def detect(df: pd.DataFrame, ticker: str, name: str) -> list[Signal]:
    """Detect rising wedge (bullish) patterns."""
    ...
    for end_idx in range(MIN_DAYS + 5, len(df) - 1):
        ...
        # Peaks must be ascending (each higher than previous)
        if not bool(np.all(np.diff(peak_prices) > 0)):
            continue

        # Troughs must be ascending (each higher than previous)
        ...
        # Lower slope must exceed upper slope (convergence upward)
        if lower_slope <= upper_slope:
            continue

        # Compute projected upper trendline at end_idx
        projected_upper = upper_slope * end_idx + upper_intercept
        if projected_upper <= 0:
            continue

        # Require breakout above projected upper trendline
        breakout_start = end_idx + 1
        if not neckline_broken_up(df, projected_upper, breakout_start):
            continue

        bo_idx = first_breakout_up_idx(df, projected_upper, breakout_start)
        if bo_idx is None:
            continue

        # Convergence quality → confidence
        slope_diff_ratio = (lower_slope - upper_slope) / upper_slope
        ...
        # Key points: first peak, first trough, last peak, last trough, breakout
        ...
        # One signal per window position — advance by half window to avoid duplicates
        break
